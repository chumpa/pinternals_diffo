...
public class PiEntity {
	...
	synchronized public void addUpdateQueue(List<PiObject> coming) {
		...
		for (PiObject n: coming) {
			if (!n.inupdatequeue && (n.is_dirty || (!n.deleted && !n.payloadexist) ) ) {
				n.inupdatequeue = true;
				updateQueue.add(n); 
			} else {
				log.config("Not for update queue" + n);
			}
		}
	}
	...
	protected HTask collectRA(PiHost p) throws MalformedURLException, IOException {
		...
		HTask h = new HTask(p.sid, "CollectRA("+intname+")", 
	...
	HTask makeOnlineHTask (PiHost p, boolean deleted) throws MalformedURLException, IOException {
		...
		HTask h = new HTask(p.sid, nm, p.establishPOST(side.url(p.uroot), true), qry);
		return h;
	}
}
...
class PiObject {
	...
	public String toString() {
		...
		s = e.title + "_" + refDB + ":" + refSWCV + (deleted?" DELETED":"") + ",payloadexist:" + payloadexist + " {" + UUtil.getStringUUIDfromBytes(objectid) + ", " + UUtil.getStringUUIDfromBytes(versionid) + "} " 
				+ s;
		return s;
	}
